...
test.describe('BookClub App - Authentication Flow', () => {
  ...
  test('should handle user registration flow', async ({ page }) => {
    ...
    await page.fill('input[name="confirmPassword"][type="password"]', 'password123');
    
    // Submit the form - using the actual button text "Create account"
    ...
    const hasError = await page.locator('.text-red-700, :has-text("error"), :has-text("failed")').isVisible();
  ...
  test('should handle existing email registration error', async ({ page }) => {
    ...
    // Submit the form - using the actual button text
    await page.click('button[type="submit"]:has-text("Create account")');
    
    // Wait for error message to appear
    await expect(page.locator('.text-red-700, :has-text("email already exists"), :has-text("user already exists")')).toBeVisible({ timeout: 5000 });
  });
  ...
  test('should handle invalid login credentials', async ({ page }) => {
    ...
    // Wait for error message to appear
    await expect(page.locator('.text-red-700, :has-text("Invalid credentials"), :has-text("incorrect"), :has-text("wrong")')).toBeVisible({ timeout: 5000 });
  });
  ...
  test('should validate email format in registration', async ({ page }) => {
    ...
    // Submit the form - using actual button text
    await page.click('button[type="submit"]:has-text("Create account")');
